...
MONGO_URL        <- readline("Please enter the MongoDB Atlas connection string: ")
MONGO_DB         <- "stat2630"
MONGO_COLLECTION <- "stock_benford"

cat("Fetching stock data from Yahoo Finance...\n")
...
fetch_yahoo <- function(symbol) {
  tryCatch({
    ...
    rownames(df) <- NULL
    cat(sprintf("  ✓ %s fetched successfully (%d rows)\n", symbol, nrow(df)))
    return(df)
  }, error = function(e) {
    cat(sprintf("  ✗ %s fetch failed: %s\n", symbol, e$message))
    return(NULL)
  })
}
...
cat(sprintf("\nAll stock data fetched successfully, total %d rows\n", nrow(all_stocks)))
...
tryCatch({
  ...
  cat(sprintf("✓ Raw data written to collection '%s_raw' (%d rows)\n",
              MONGO_COLLECTION, nrow(all_stocks)))
}, error = function(e) {
  cat(sprintf("✗ MongoDB write failed: %s\n", e$message))
})
...
for (sym in STOCKS) {
  for (col in c("close", "volume")) {
    ...
    cat(sprintf("\n======== %s ========\n", label))
    r1   <- run_chisq(cnt1,   Benford_1,   "First digit (decimal)")
    r12  <- run_chisq(cnt12,  Benford_12,  "First two digits (decimal)")
    r123 <- run_chisq(cnt123, Benford_123, "First three digits (decimal)")
    r2   <- run_chisq(cnt2,   Benford_2,   "Second digit (decimal)")
    r3   <- run_chisq(cnt3,   Benford_3,   "Third digit (decimal)")
    ro1  <- run_chisq(cnto1,  Benford_o1,  "First digit (octal)")
    rh1  <- run_chisq(cnth1,  Benford_h1,  "First digit (hexadecimal)")

    mad_val <- calc_mad(cnt1, Benford_1)
    cat(sprintf("  MAD (first digit): %.6f", mad_val))
    if      (mad_val < 0.006) cat(" -> Conforms to Benford ✓\n")
    else if (mad_val < 0.012) cat(" -> Slight deviation\n")
    else if (mad_val < 0.015) cat(" -> Marginal deviation\n")
    else                      cat(" -> Significant deviation\n")
    ...
    grid.arrange(p1,p2,p3,p4,p5, layout_matrix=lay1,
                 ...
                 top=sprintf("%s — Decimal Benford Analysis", label))

    lay2 <- rbind(c(1,2,3))
    grid.arrange(p1,p6,p7, layout_matrix=lay2,
                 widths=c(1,1,1), heights=c(1,1),
                 top=sprintf("%s — Base Comparison", label))
...
tryCatch({
  ...
  cat(sprintf("\n✓ Test results written to MongoDB (%d rows)\n", nrow(results_df)))

  check <- con_res$find('{}', limit=3)
  cat("✓ Verification read-back successful:\n")
  print(check[, c("symbol","column","chi_1","p_1","mad_1")])
}, error = function(e) {
  cat(sprintf("✗ Failed to write MongoDB results: %s\n", e$message))
})

cat("\nPipeline completed: Yahoo Finance -> R cleaning -> MongoDB Atlas -> Benford analysis -> Results saved to Atlas\n")
